...
sbs() {
  if [[ "${1:-}" == "--note" ]]; then
    ...
    [[ -n "$note" ]] || { printf 'Uso: sbs [--note <ruta>] <tarea>\n' >&2; return 2; }
    _sb_ai_session start --task "$*" --project-note "$note" --copy
  else
    [[ -n "$*" ]] || { printf 'Uso: sbs <tarea>\n' >&2; return 2; }
...
sbl() {
  local note=""; if [[ "${1:-}" == "--note" ]]; then note="${2:-}"; shift 2; fi
  [[ -n "$*" ]] || { printf 'Uso: sbl [--note <ruta>] <tarea>\n' >&2; return 2; }
  if [[ -n "$note" ]]; then
...
sbe() {
  [[ -n "$*" ]] || { printf 'Uso: sbe [--from <legacy>] <resumen>\n' >&2; return 2; }
...
afapplylast() {
  local draft; draft="$(aflastdraft)" || return 1
  [[ -n "$draft" ]] || { printf 'No encontré drafts de distill.\n' >&2; return 1; }
...
ai-session-runbook() {
  ...
  [[ -f "$runbook" ]] || { printf 'No se encontró el runbook: %s\n' "$runbook" >&2; return 1; }
