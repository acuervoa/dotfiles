...
cpu_col() {
  local v=$1
  ((v >= 80)) && echo "$RED" || { ((v >= 50)) && echo "$AMBER" || echo "$GREEN"; }
}
temp_col() {
  local v=$1
  ((v >= 85)) && echo "$RED" || { ((v >= 70)) && echo "$AMBER" || echo "$GREEN"; }
}
load_col() {
  local v=$1
  ((v >= 100)) && echo "$RED" || { ((v >= 70)) && echo "$AMBER" || echo "$GREEN"; }
}
mem_col() {
  local v=$1
  ((v >= 80)) && echo "$RED" || { ((v >= 60)) && echo "$AMBER" || echo "$GREEN"; }
}
net_col() {
  local mi10=$1
  ((mi10 >= 50 * 10)) && echo "$RED" || { ((mi10 >= 10 * 10)) && echo "$AMBER" || echo "$GREEN"; }
}
...
human_rate_fixed() {
  ...
  if ((kis >= 1024 * 1024)); then
    unit="Gi"
    val=$(awk -v v="$kis" 'BEGIN{printf "%.1f", v/1048576.0}')
  elif ((kis >= 1024)); then
    unit="Mi"
    val=$(awk -v v="$kis" 'BEGIN{printf "%.1f", v/1024.0}')
  else
    unit="Ki"
    val=$(awk -v v="$kis" 'BEGIN{printf "%.1f", v+0.0}')
  fi
...
LOAD=$(load_pct)
LOAD_STR=$(pad_pct "$LOAD")
LOAD_COL=$(load_col "$LOAD")
MEM=$(mem_pct)
MEM_STR=$(pad_pct "$MEM")
MEM_COL=$(mem_col "$MEM")
