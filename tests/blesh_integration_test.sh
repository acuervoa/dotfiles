...
mkdir -p "$tmp_home/.config" "$tmp_home/state" "$tmp_home/cache" "$tmp_runtime"
...
timeout 20 script -qefc "env HOME='$tmp_home' XDG_STATE_HOME='$tmp_home/state' XDG_CACHE_HOME='$tmp_home/cache' XDG_RUNTIME_DIR='$tmp_runtime' TERM=xterm-256color bash --noprofile --rcfile '$repo_root/stow/bash/.bashrc' -i" "$tmp_output" <"$tmp_home/input" >/dev/null 2>&1
