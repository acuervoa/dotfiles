...
grep -Fq 'bindsym $mod+d exec --no-startup-id rofi -show drun' "$i3_config"
! grep -Fq 'bindsym $mod+d exec --no-startup-id rofi -modi run -show drun' "$i3_config"
grep -Fq '.local/state/rofi/mod-d.log' "$i3_config"
grep -Fq '2>&1' "$i3_config"
