...
grep -Fq 'pane-border-status top' "$misc_lib"
grep -Fq 'pane-border-format' "$misc_lib"
grep -Fq 'select-pane -t "$name:dev.0" -T "EDITOR"' "$misc_lib"
grep -Fq 'select-pane -t "$name:dev.1" -T "LOGS"' "$misc_lib"
grep -Fq 'select-pane -t "$name:dev.2" -T "SHELL"' "$misc_lib"
grep -Fq 'editor_cmd="${editor_cmd} ."' "$misc_lib"
