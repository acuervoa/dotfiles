...
- **Librería Bash modular** (`stow/bash/.bash_lib/*.sh`) con helpers para git, docker, navegación, productividad y flujos AI: clipboard con fallback OSC52 (`cb`), búsqueda de archivos (`fo`), tmux helper `ts`, docker compose helpers (`dorebuild`, `dsh`, `dlogs`), accesos rápidos de `ai-flow` (`af`, `afl`, `afx`) y lanzadores AI (`gpt`, `ia`, `ia-code`).
...
- **Bash library** (`stow/bash/.bash_lib`): core (confirm, req, tmux `ts`), nav (`fo` con excludes/auto-cd, `cb` con OSC52), git (`gp` seguro, `ggraph`, `glast`), docker (`docps`, `dlogs`, `dsh`, `dorebuild`), misc (`fhist`, `envswap`, `dev`, `qa`, `rtest`, `rserve`, `rqa`), ai (`af`, `afl`, `afx`, `afapplylast`, `gpt`, `ia`, `ia-code`).
